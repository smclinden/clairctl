...
var log = capnslog.NewPackageLogger("github.com/ids/clairctl", "dockercli")
...
func GetLocalManifest(imageName string, withExport bool) (reference.NamedTagged, distribution.Manifest, error) {
	...
	log.Debugf("supplied image name: %v", imageName)
	...
	log.Debugf("reference namedtagged: %v", reference.WithDefaultTag(n).(reference.NamedTagged))
	...
	if withExport {
		...
		manifest, err = save(imageName)
	} else {
		//manifest, err = historyFromCommand(image.Name()) // + ":" + image.Tag())
		manifest, err = historyFromCommand(imageName)
	}
...
func save(imageName string) (distribution.Manifest, error) {
	path := config.TmpLocal() + "/" + strings.Replace(imageName, ":", "_", -1) + "/blobs"
